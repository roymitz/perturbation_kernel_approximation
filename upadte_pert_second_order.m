function [Q_pert, D_pert] = upadte_pert_second_order(Q0, D0, A, dA, mu)
...
%     Q_pert = normc(Q_pert);
%     [~, idx] = sort(diag(D_pert), 'descend'); idx = idx(1:n_eigs_each_block);
%     idx = 1:n_eigs_each_block;
%     D_pert = D_pert(idx, idx);
%     Q_pert = Q_pert(:, idx);
%       [U,S,~] = svds( Q_pert * D_pert, n_eigs_each_block);
%       Q_pert = U;
%       D_pert = S;
end
